#!/bin/bash
# Script para ver logs del startup script en la VM
set -euo pipefail
...
# Determinar comando tail según parámetros
if [ "$FOLLOW" = "true" ] || [ "$FOLLOW" = "1" ]; then
  TAIL_CMD="tail -f"
  echo -e "${GREEN}Siguiendo logs del startup script en tiempo real (Ctrl+C para salir)...${NC}"
else
  TAIL_CMD="tail -${LINES}"
  echo -e "${GREEN}Obteniendo últimas ${LINES} líneas del startup script...${NC}"
fi

# Determinar método de conexión SSH (prioridad: .pem > .json > IAP)
# Si hay .pem e IP pública, usar ssh directamente
# NOTA: tail -f es interactivo y nunca termina, así que no usamos timeout
if [ -n "$SSH_KEY_PATH" ] && [ -f "$SSH_KEY_PATH" ] && \
  ...
  # Si falla la conexión inicial (ConnectTimeout), entonces intentar con IAP
  ssh -i "$SSH_KEY_PATH" \
    -o StrictHostKeyChecking=no \
    -o UserKnownHostsFile=/dev/null \
    -o ConnectTimeout=5 \
    "$SSH_USER@$EXTERNAL_IP" \
    "${TAIL_CMD} /var/log/startup-script.log" 2>&1
  SSH_EXIT_CODE=$?
...
gcloud compute ssh "$SSH_USER@$INSTANCE_NAME" \
  ...
  --project="$PROJECT_ID" \
  $SSH_OPTS \
  --command="${TAIL_CMD} /var/log/startup-script.log" 2>&1
